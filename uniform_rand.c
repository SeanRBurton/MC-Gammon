...
Move select_move(Board* b, Color c, u8 die0, u8 die1, u32 time_limit, bool* die0_used) {
  ...
  b1.red = red;
  int playouts = 0;
  int playout_block = 256;
  u64 t1 = t0;
  u64 m  = random_number() % moves;
  if (die0_used != NULL) {
      *die0_used = m < moves0;
  }
...
int main(void) {
  ...
  while (1) {
    if (c == our_color) {
      if (die0 == die1) {
        for (int i = 0; i < 4; i++) {
          Move move = select_move(&b, c, die0, 255, 1000 / 4, NULL); // TOdo: pass?
        ...
        for(int i = 0; i < 2; i++) {
          ...
          //fprintf(log_file, "dice: %d %d\n", die0, die1);
          Move move = select_move(&b, c, die0, die1, 1000 / 2, &die0_used);
